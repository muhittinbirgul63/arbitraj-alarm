"""
Kripto Arbitraj Alarm Botu v5.1
- Binance, Gate, MEXC, OKX, KuCoin (USDT)
- Paribu, BTCTürk (TL)
- Paribu fiyatları WebSocket üzerinden (HTTP fallback)
- Orderbook doğrulama
...
# WebSocket opsiyonel — yoksa HTTP fallback'e geçer
try:
    import websocket
    WEBSOCKET_VAR = True
except ImportError:
    WEBSOCKET_VAR = False
    print("[UYARI] websocket-client kurulu değil. 'pip install websocket-client' ile kur. HTTP kullanılacak.")
...
# ─── PARİBU WEBSOCKET CACHE ──────────────────────────────────────────────────
# {coin: {"fiyat": x, "bid": y, "ask": z, "hacim": h, "ts": timestamp}}
_paribu_ws_cache      = {}
_paribu_ws_cache_lock = threading.Lock()
_paribu_ws_last_update = 0   # Son veri zamanı
_paribu_ws_active      = False  # WS bağlı mı?

# Paribu'da listelenen coin listesi (WS aboneliği için)
# Boşsa HTTP ticker'dan öğrenilir, sonra WS subscribe edilir
_paribu_ws_coinler = []
...
def paribu_tumfiyatlar():
    """Önce WebSocket cache'e bak, bayatsa HTTP'ye düş."""
    global _paribu_ws_active
    if _paribu_ws_active and time.time() - _paribu_ws_last_update < 10:
        with _paribu_ws_cache_lock:
            sonuc = {}
            simdi = time.time()
            for coin, veri in _paribu_ws_cache.items():
                if simdi - veri.get("ts", 0) < 30:
                    sonuc[coin] = {
                        "fiyat": veri["fiyat"],
                        "ask":   veri.get("ask", veri["fiyat"]),
                        "bid":   veri.get("bid", veri["fiyat"]),
                        "hacim": veri["hacim"],
                    }
            if sonuc:
                borsa_hata_kontrol("Paribu", True)
                return sonuc
    # WS bayat veya yok — HTTP fallback
    return _paribu_tumfiyatlar_http()


def _paribu_tumfiyatlar_http():
    """HTTP fallback: www.paribu.com/ticker → api.paribu.com/ticker"""
    try:
        r = None
        for url in ["https://www.paribu.com/ticker", "https://api.paribu.com/ticker"]:
            try:
        ...
        # WS henüz coin listesi bilmiyorsa HTTP'den öğren
        global _paribu_ws_coinler
        if not _paribu_ws_coinler and sonuc:
            _paribu_ws_coinler = list(sonuc.keys())

        borsa_hata_kontrol("Paribu", True)
        return sonuc
    except Exception as e:
        print(f"Paribu HTTP hata: {e}")
        borsa_hata_kontrol("Paribu", False)
        return {}
...
def _paribu_ws_on_message(ws, message):
    global _paribu_ws_last_update
    try:
        ...
        if tip == "ticker24h":
            sembol = veri.get("s", "")
            r      = veri.get("r", {})
            if sembol.endswith("_tl"):
                coin = sembol.replace("_tl", "").upper()
                try:
                    fiyat = float(r.get("c", 0))   # close (son fiyat)
                    hacim = float(r.get("q", 0))   # quote volume (TL)
                    if fiyat > 0:
                        with _paribu_ws_cache_lock:
                            mevcut = _paribu_ws_cache.get(coin, {})
                            _paribu_ws_cache[coin] = {
                                "fiyat": fiyat,
                                "hacim": hacim,
                                "bid":   mevcut.get("bid", fiyat),
                                "ask":   mevcut.get("ask", fiyat),
                                "ts":    time.time(),
                            }
                        _paribu_ws_last_update = time.time()
                except:
                    pass
            ...
            if sembol.endswith("_tl"):
                ...
                try:
                    ...
                    if bid > 0 or ask > 0:
                        with _paribu_ws_cache_lock:
                            mevcut = _paribu_ws_cache.get(coin, {})
                            mevcut["bid"] = bid or mevcut.get("bid", 0)
                            mevcut["ask"] = ask or mevcut.get("ask", 0)
                            mevcut["ts"]  = time.time()
                            _paribu_ws_cache[coin] = mevcut
                        _paribu_ws_last_update = time.time()
                except:
                    pass
    except:
...
def _paribu_ws_on_open(ws):
    global _paribu_ws_active
    _paribu_ws_active = True
    print("[WS] ✅ Paribu WebSocket bağlandı")

    coinler = _paribu_ws_coinler or ["USDT", "BTC", "ETH", "SOL", "XRP", "BNB", "DOGE", "ADA", "TRX", "AVAX"]
    batch = 50
    for i in range(0, len(coinler), batch):
        grup    = coinler[i:i + batch]
        kanallar = []
        for coin in grup:
            kanallar.append(f"ticker24h:{coin.lower()}_tl@1000ms")
            kanallar.append(f"orderbook:{coin.lower()}_tl@1000ms")
        try:
            ws.send(json.dumps({
                "method":   "subscribe",
                "channels": kanallar,
            ...
            print(f"[WS] Subscribe hatası: {e}")
            break
    print(f"[WS] {len(coinler)} pariteye abone olundu")


def _paribu_ws_on_error(ws, error):
    global _paribu_ws_active
    _paribu_ws_active = False
    print(f"[WS] Hata: {error}")


def _paribu_ws_on_close(ws, status, msg):
    global _paribu_ws_active
    _paribu_ws_active = False
    print(f"[WS] Kapandı: {status}")


def paribu_ws_baslat():
    """Arka planda Paribu WebSocket başlatır, kopunca otomatik reconnect."""
    if not WEBSOCKET_VAR:
        print("[WS] websocket-client yok, HTTP kullanılacak")
        return
        ...
        while True:
            try:
                ws = websocket.WebSocketApp(
                    ...
                    on_open=_paribu_ws_on_open,
                    on_message=_paribu_ws_on_message,
                    on_error=_paribu_ws_on_error,
                    on_close=_paribu_ws_on_close,
                )
                ws.run_forever(ping_interval=30, ping_timeout=10)
            except Exception as e:
                print(f"[WS] Thread hatası: {e}")
            print("[WS] 5 saniye sonra tekrar bağlanılacak...")
            time.sleep(5)
...
def bot_calistir():
    ...
    print("Arbitraj Alarm Botu v5.1 başlatılıyor...")

    komut_thread = threading.Thread(target=komut_dinleyici, daemon=True)
    komut_thread.start()

    # Paribu coin listesini öğrenmek için ilk HTTP çekimi
    print("[WS] Paribu coin listesi öğreniliyor...")
    _paribu_tumfiyatlar_http()

    # WebSocket başlat
    paribu_ws_baslat()

    ws_str = "✅ WS aktif" if WEBSOCKET_VAR else "⚠️ WS yok (HTTP)"
